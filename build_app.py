...
<!-- Tab icon: a serif V, drawn as a path rather than <text> so it does not
     ...
     flashcards.html stays a single self-contained file. -->
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect width='64' height='64' rx='14' fill='%230d8fa8'/%3E%3Cpath d='M11 17H29V22H25.5L34 39L43 22H39V17H53V22H48L35.4 49H31L16 22H11Z' fill='%23fff'/%3E%3C/svg%3E">
...
/* iswearenglish links, sitting inside the Means section between the one-line
   ...
   Two fixed columns rather than auto-fit, so a word with one video gets a
   half-width thumbnail instead of a full-width slab. */
.vgrid{display:grid;grid-template-columns:1fr 1fr;gap:.6rem;margin:.85rem 0 1.15rem}
.vgrid a{text-decoration:none;color:inherit}
.vgrid img{width:100%;height:auto;aspect-ratio:16/9;object-fit:cover;display:block;
  border-radius:9px;background:var(--panel2);border:1px solid var(--line)}
.vgrid a:hover img{border-color:var(--accent)}
.vgrid .c{font-family:var(--ui);font-size:.78rem;font-weight:600;color:var(--accent);
...
/* Scope picker. 693 tickable scopes is far too many for ordinary stacked
   checkbox rows, so it is a dense, searchable, self-scrolling panel. */
.seclabel{margin:1.2rem 0 .4rem;font-size:.7rem;letter-spacing:.13em;
  text-transform:uppercase;color:var(--accent);font-weight:700}
.scopes{border:1px solid var(--line);border-radius:var(--radius);overflow:hidden}
.scopebar{display:flex;gap:.5rem;align-items:center;flex-wrap:wrap;
  padding:.5rem .6rem;border-bottom:1px solid var(--line);background:var(--panel2)}
.scopebar input[type=search]{flex:1 1 9rem;min-width:7rem}
.scopetabs{display:flex;gap:.3rem;flex-wrap:wrap}
.scopetab{font-size:.76rem;font-family:inherit;border:1px solid var(--line);
  border-radius:20px;padding:.2rem .7rem;cursor:pointer;color:var(--ink2);
  background:var(--panel)}
.scopetab.on{background:var(--accent-soft);border-color:var(--accent);
  color:var(--accent);font-weight:600}
.scopelist{max-height:44vh;overflow:auto;background:var(--panel)}
.scopelist label{display:flex;gap:.55rem;align-items:center;margin:0;
  padding:.4rem .7rem;border-bottom:1px solid var(--line);font-size:.9rem}
.scopelist label:last-child{border-bottom:0}
.scopelist label:hover{background:var(--panel2)}
.scopelist input[type=checkbox]{margin:0;flex:0 0 auto}
.scopelist .st{flex:0 0 auto;min-width:5.6rem;text-align:center;font-size:.64rem;
  letter-spacing:.07em;text-transform:uppercase;color:var(--accent);
  border:1px solid var(--line);border-radius:4px;padding:.08rem .3rem}
.scopelist .sn{flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.scopelist .sc{flex:0 0 auto;color:var(--dim);font-size:.78rem;min-width:2rem;text-align:right}
.scopefoot{padding:.5rem .7rem;border-top:1px solid var(--line);
  background:var(--panel2);color:var(--dim);font-size:.82rem}
.linkish{background:none;border:0;padding:0;font-family:inherit;font-size:.78rem;
...
/* ---- selectable quiz scopes ---------------------------------------------
   Two unrelated taxonomies are quizzable: the 38 GregMat lists a card carries
   in c.groups, and the semantic clusters in GROUPS. The picker lets them be
   mixed freely, so they are normalised into one shape here - {id,kind,title,
   words:[{word}]} - and everything downstream stops caring which is which.
   List ids are prefixed so they can never collide with a group slug. */
const LISTS = (() => {
  const by = new Map();
  CARDS.forEach(c => (c.groups || []).forEach(n => {
    if(!by.has(n)) by.set(n, []);
    by.get(n).push({word: c.word});
  }));
  return [...by.keys()].sort((a, b) => a - b).map(n => ({
    id: "list:" + n, kind: "list", title: "GregMat list " + n, words: by.get(n),
  }));
})();
const SCOPES = [...LISTS, ...GROUPS];
const SCOPE_BY_ID = {};
SCOPES.forEach(s => { SCOPE_BY_ID[s.id] = s; });
const scopeById = id => SCOPE_BY_ID[id] || null;
...
function md(src){
  ...
  return blocks.map(b => {
   ...
   // [^\n]+? rather than [^*]+ : bold routinely contains italics, as in
   // "**Beguile hides *guile* behind a *smile*.**". Excluding * meant those
   // never matched and 206 fields rendered with literal asterisks. Newlines
   // stay excluded so an unpaired ** cannot bleed across blocks.
   .replace(/\*\*([^\n]+?)\*\*/g, "<strong>$1</strong>")
   .replace(/(^|[^*])\*([^*\n]+)\*/g, "$1<em>$2</em>")
...
function videoLabel(word, v){
  const others = v.subjects.filter(s => s.toLowerCase() !== word.toLowerCase());
  if(v.kind === "dedicated" || !others.length)
    return v.subjects.find(s => s.toLowerCase() === word.toLowerCase()) || v.subjects[0];
  return "with " + (others.length > 4
...
function videoHTML(word){
  ...
  if(!vids.length) return `<div class="vids"><a class="vchip dim" target="_blank" rel="noopener"
      href="${VIDEO_SEARCH}${encodeURIComponent(word)}"
      title="No video matched this word - search the channel">&#9654; search iswearenglish</a></div>`;
  // The channel puts the word itself in the thumbnail, in large type, so these
  // read as a summary of the video rather than as decoration. onerror hides a
  // broken image offline and leaves the label, which still links.
  return `<div class="vgrid">` + vids.map(v => `<a href="https://youtu.be/${esc(v.id)}"
      target="_blank" rel="noopener" title="${esc(v.subjects.join(" · "))}">
      <img loading="lazy" width="320" height="180" alt=""
           src="https://i.ytimg.com/vi/${esc(v.id)}/mqdefault.jpg"
           onerror="this.style.display='none'">
      <div class="c">${esc(videoLabel(word, v))}</div></a>`).join("") + `</div>`;
}
...
/* Distractors in strict preference order, each tier exhausted before the next:
   any group the word itself belongs to, then anywhere in the deck.
   Shuffling the tiers together would let a random word beat an in-cluster one,
   which is exactly what makes a question easy.

   Deliberately independent of the quiz scope: wrong options are drawn from the
   whole vocabulary, so selecting a handful of groups narrows which words get
   ASKED without narrowing what they compete against. */
function distractors(card, n){
  ...
  const kin = groupsFor(card.word).flatMap(g => g.words.map(m => m.word));

  const out = [], seen = new Set();
  for(const w of take(kin, seen)){
    if(out.length >= n) break;
    out.push(w); seen.add(w.toLowerCase());
  }
  for(let i = 0; out.length < n && i < 400; i++){   // then anywhere in the deck
...
/* The words of every selected scope, or null when quizzing everything.
   Collecting into a Set first is what shuffles the selection together: a word
   in three of the chosen groups is one entry, not three, so no group is
   over-weighted and questions arrive in no particular group order. */
function scopedCards(){
  if(!quizSel.length) return null;
  const want = new Set();
  quizSel.forEach(id => (scopeById(id)?.words || [])
    .forEach(m => want.add(m.word.toLowerCase())));
  if(!want.size) return null;
  return CARDS.filter(c => want.has(c.word.toLowerCase()));
}
/* How the current scope reads in a heading. */
function scopeLabel(){
  if(!quizSel.length) return "";
  if(quizSel.length === 1) return scopeById(quizSel[0])?.title || "";
  return quizSel.length + " groups";
}
/* Nuance, odd-one-out and intensity questions already name their group, so
   appending a one-group scope would print the same title twice. */
function scopeSuffix(label){
  const s = scopeLabel();
  if(!s || (label || "").toLowerCase().includes(s.toLowerCase())) return "";
  return " · " + esc(s);
}
...
/* The chosen scopes, persisted so the selection survives a reload and does not
   have to be encoded into the URL - twenty group slugs would be unreadable. */
const QSEL_KEY = "gre-vocab-qscope-v1";
let quizSel = (JSON.parse(localStorage.getItem(QSEL_KEY) || "[]"))
                .filter(id => SCOPE_BY_ID[id]);   // drop ids from an older build
let quizSelAtStart = "";
let q = null, qAnswered = false, qScore = {right:0, total:0}, qStarted = false;
const saveSel = () => localStorage.setItem(QSEL_KEY, JSON.stringify(quizSel));

function nextQuestion(){
  let pool = scopedCards() || filtered();
  // Spaced repetition still applies inside a selection, but a small selection
  // can easily have nothing due - fall back to the whole scope rather than
  // dead-ending on an empty screen.
...
/* ---- scope picker ------------------------------------------------------
   693 selectable scopes is far too many for a plain checkbox list, so the
   picker is searchable and the tabs filter what is shown without touching
   what is selected - a GregMat list and a meaning cluster can be ticked in
   the same quiz. The list re-renders on its own so typing in the filter does
   not rebuild the whole setup screen underneath it. */
let scopeTab = "all", scopeQuery = "";

function scopeMatches(){
  const q = scopeQuery.trim().toLowerCase();
  return SCOPES.filter(s => {
    if(scopeTab === "list" && s.kind !== "list") return false;
    if(scopeTab === "sem"  && s.kind === "list") return false;
    if(!q) return true;
    return (s.title + " " + s.words.map(m => m.word).join(" ")).toLowerCase().includes(q);
  });
}
...
function selectedWords(){
  const w = new Set();
  quizSel.forEach(id => (scopeById(id)?.words || [])
    .forEach(m => w.add(m.word.toLowerCase())));
  return w;
}

function renderScopeFoot(){
  const f = $("scopefoot");
  if(!f) return;
  const n = quizSel.length, w = selectedWords().size;
  f.innerHTML = n
    ? `<strong>${n}</strong> group${n === 1 ? "" : "s"} ·
       <strong>${w}</strong> distinct word${w === 1 ? "" : "s"} · shuffled together`
    : `Nothing selected — the quiz draws on all ${CARDS.length} words.`;
}

function renderScopeList(){
  const list = scopeMatches(), sel = new Set(quizSel);
  const box = $("scopelist");
  if(!box) return;
  box.innerHTML = list.length
    ? list.map(s => `<label data-sid="${esc(s.id)}">
        <input type="checkbox" class="sx" ${sel.has(s.id) ? "checked" : ""}>
        <span class="st">${esc(s.kind === "list" ? "list" : (KINDLABEL[s.kind] || s.kind))}</span>
        <span class="sn">${esc(s.title)}</span>
        <span class="sc">${s.words.length}</span></label>`).join("")
    : `<div class="hint" style="padding:1rem">No group matches that.</div>`;
  box.querySelectorAll("label").forEach(l => l.querySelector(".sx").onchange = e => {
    const id = l.dataset.sid;
    quizSel = e.target.checked ? [...new Set([...quizSel, id])]
                               : quizSel.filter(x => x !== id);
    saveSel(); renderScopeFoot();
  });
...
function renderQuiz(){
  ...
  if(!qStarted){
    ...
    const TABS = [["all","All"],["list","GregMat lists"],["sem","Semantic groups"]];
    el.innerHTML = `<div class="q"><div class="setup">
      <h3>Quiz</h3>
      <p class="hint">${dueCount()} of ${CARDS.length} words are due for review today.</p>
      <div class="seclabel">Scope — tick any number of groups</div>
      <div class="scopes">
        <div class="scopebar">
          <div class="scopetabs">${TABS.map(([k,t]) =>
            `<button class="scopetab ${scopeTab===k?"on":""}" data-tab="${k}">${t}</button>`).join("")}</div>
          <input type="search" id="sq" placeholder="Filter groups" value="${esc(scopeQuery)}">
          <button class="linkish" id="sall">select all <span id="scount"></span></button>
          <button class="linkish" id="sclr">clear</button>
        </div>
        <div class="scopelist" id="scopelist"></div>
        <div class="scopefoot" id="scopefoot"></div>
      </div>
      <label><input type="checkbox" id="qdue" ${quizOnlyDue?"checked":""}>
        <span><strong>Only ask what's due</strong><br><span class="hint">Spaced repetition. If nothing in your selection is due, it quizzes the whole selection anyway.</span></span></label>
      <hr class="sep">
    ...
    renderScopeList();
    el.querySelectorAll(".scopetab").forEach(b => b.onclick = () => {
      scopeTab = b.dataset.tab;
      el.querySelectorAll(".scopetab").forEach(x => x.classList.toggle("on", x === b));
      renderScopeList();
    });
    $("sq").oninput = e => { scopeQuery = e.target.value; renderScopeList(); };
    // "select all" adds only what is currently shown, so it composes with the
    // filter instead of dumping all 693 scopes in.
    $("sall").onclick = () => {
      quizSel = [...new Set([...quizSel, ...scopeMatches().map(s => s.id)])];
      saveSel(); renderScopeList();
    };
    $("sclr").onclick = () => { quizSel = []; saveSel(); renderScopeList(); };
  ...
  let h = `<div class="q">
    <div class="qhead"><span class="qtype">${esc(q.label)}${scopeSuffix(q.label)}
        <button class="linkish" id="qrescope" title="Change which groups this quiz draws on">change</button></span>
  ...
  const rs = $("qrescope");
  if(rs) rs.onclick = () => { qStarted = false; renderQuiz(); };
  const fr = $("qfresh");
...
/* ==========================================================================
   ...
   /quiz/<groupId> is a shortcut, not a separate mode: it sets the scope to
   that one group and rewrites itself to /quiz. The selection lives in
   localStorage rather than the URL because twenty group slugs in a hash would
   be unreadable, and it has to survive a reload anyway.
   ========================================================================== */
...
function applyRoute(route){
  ...
  // A group id in the quiz route means "quiz exactly this one" - it replaces
  // whatever was selected, then drops out of the URL.
  if(m === "quiz" && arg && SCOPE_BY_ID[arg]){
    quizSel = [arg]; saveSel();
    route = "/quiz";
  ...
  // from the old selection.
  if(m === "quiz" && (prevMode !== "quiz" || quizSel.join("|") !== quizSelAtStart))
    qStarted = false;

  localStorage.setItem(ROUTE_KEY, route);
  render();
  if(arg && (m === "browse" || m === "groups")) setTimeout(() => {
